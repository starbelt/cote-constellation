#!/usr/bin/env python3
"""Generate combined missed connection matrix (satellites in view but not connected) using visibility logs across all image sizes and satellite counts"""
...
def calculate_missed_connection_from_visibility_log(policy_dir):
    """Calculate missed connection opportunities from visibility_log.csv using efficient event-based approach
    
    Returns:
        connection_efficiency: Percentage of in-view time that was connected (0-100)
        missed_events: Count of events where in_view=1 AND connected=0
    ...
    try:
        ...
        # Calculate missed connection events: in_view=1 AND connected=0
        missed_events = df[(df['in_view'] == 1) & (df['connected'] == 0)]
        
        # Count total in-view events
        total_in_view_events = df[df['in_view'] == 1]
        
        # Calculate connection efficiency: what % of in-view time was actually connected?
        if len(total_in_view_events) > 0:
            connected_events = df[(df['in_view'] == 1) & (df['connected'] == 1)]
            connection_efficiency = (len(connected_events) / len(total_in_view_events)) * 100
        else:
            connection_efficiency = 0
        
        return connection_efficiency, len(missed_events), len(total_in_view_events)
        
    except Exception as e:
        print(f"Error processing visibility log: {e}")
        return 0, 0, 0

def calculate_missed_connection_for_strategy(strategy_folder):
    """Calculate missed connection metrics for all policies in a strategy using visibility logs"""
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        for policy in policies:
            ...
            if policy_dir.exists():
                # Calculate missed connection metrics using efficient visibility log approach
                efficiency, missed_count, total_in_view = calculate_missed_connection_from_visibility_log(policy_dir)
                efficiencies[policy] = efficiency
                missed_counts[policy] = missed_count
                total_in_view_counts[policy] = total_in_view
            else:
                efficiencies[policy] = 0
                missed_counts[policy] = 0
                total_in_view_counts[policy] = 0
    
    return efficiencies, missed_counts, total_in_view_counts
...
def generate_combined_missed_connection_matrix_4d(base_folder):
    """Generate 4D missed connection matrix across all image sizes and satellite counts"""
    ...
    print(f"=== Generating 4D Missed Connection Matrix ===")
    ...
    efficiency_matrix = np.zeros((total_rows, total_cols))  # Connection efficiency (0-100%)
    missed_matrix = np.zeros((total_rows, total_cols))  # Missed connection events count
    ...
    for img_size in image_sizes:
        for sat_count in satellite_counts:
            ...
            for strategy_idx, strategy in enumerate(strategies):
                ...
                # Calculate missed connection metrics for this strategy
                efficiencies, missed_counts, total_in_view_counts = calculate_missed_connection_for_strategy(strategy_folder)
                
                # Show detailed progress for each policy
                for policy in policies:
                    efficiency = efficiencies[policy]
                    missed = missed_counts[policy]
                    total_in_view = total_in_view_counts[policy]
                    connected = total_in_view - missed
                    # Show format: "connected_events/total_in_view_events = X% connection efficiency"
                    print(f"   {policy}: {int(connected)}/{int(total_in_view)} connected when in view = {efficiency:.1f}% connection efficiency")
                ...
                for policy_idx, policy in enumerate(policies):
                    ...
                    efficiency_matrix[row_idx, col_idx] = efficiencies[policy]
                    missed_matrix[row_idx, col_idx] = missed_counts[policy]
    ...
    # Use Greens colormap (higher connection efficiency = better = darker green)
    im = ax.imshow(efficiency_matrix, cmap='Greens', aspect='auto', vmin=0, vmax=100)
    
    # Create the heatmap based on connection efficiency percentages
    ...
    cbar.set_label('Connection Efficiency (%)', rotation=270, labelpad=25, fontsize=14, fontweight='bold')
        ...
        for img_idx, img_size in enumerate(image_sizes):
            for strategy_idx, strategy in enumerate(strategies):
                for sat_count_idx, sat_count in enumerate(satellite_counts):
                    ...
                    efficiency = efficiency_matrix[row_idx, col_idx]
                    missed = missed_matrix[row_idx, col_idx]
                    total_in_view = total_in_view_matrix[row_idx, col_idx]
                    connected = total_in_view - missed
                    
                    # Format text showing connected/total in-view events and connection efficiency
                    value_text = f'{int(connected)}/{int(total_in_view)}\n{efficiency:.1f}%'
                        
                    text = ax.text(col_idx, row_idx, value_text, ha="center", va="center", 
                                 color='black', fontweight='bold', fontsize=10)
    
    # Create title
    title = f'Connection Efficiency (Connected Events / In-View Events)'
    ...
    # Save the plot
    output_path = "combined_missed_connection_matrix_4d.png"
    plt.savefig(output_path, dpi=300, bbox_inches='tight')
    plt.close()
    print(f"\n✅ 4D Missed Connection Matrix saved: {output_path}")
    
    # Print summary
    print(f"\n=== COMBINED MISSED CONNECTION SUMMARY ===")
    print(f"{'Policy':<12} {'Image Size':<12} {'Satellite Count':<15} {'Strategy':<20} {'Missed Events':<15} {'Connection Efficiency':<10}")
    print("-" * 110)
        ...
        for img_idx, img_size in enumerate(image_sizes):
            for strategy_idx, strategy in enumerate(strategies):
                for sat_count_idx, sat_count in enumerate(satellite_counts):
                    ...
                    if missed > 0:  # Only show entries with missed connections
                        print(f"{policy.upper():<12} {img_size:>7.3f} MB   {sat_count:<15} {strategy:<20} {int(missed):<15} {efficiency:>6.1f}%")

def main():
    parser = argparse.ArgumentParser(description='Generate combined 4D missed connection matrix (in-view but not connected) across image sizes and satellite counts using visibility logs')
    parser.add_argument('base_folder', help='Path to folder containing multiple constellation_analysis folders')
    
    args = parser.parse_args()
    
    generate_combined_missed_connection_matrix_4d(args.base_folder)
