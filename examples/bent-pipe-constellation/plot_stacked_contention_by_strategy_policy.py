#!/usr/bin/env python3
"""
Plot Stacked Bar Charts: CONTENTION TIME by Strategy & Policy
...
Y-axis: Total contention time (hours)
...
Contention Definition: in_view = 1 AND connected = 0
                       (satellite can see ground station but doesn't connect - ground station busy)
"""
...
def get_contention_time(zip_path, policy='fifo'):
    """
    Calculate total contention time from visibility_log.csv in zip file
    
    Contention Definition: in_view = 1 AND connected = 0
                          (satellite can see ground station but doesn't connect - ground station busy)
    
    Returns: Total contention time in hours
    """
    try:
        ...
        # Contention condition: in view but not connected
        contention_entries = df[(df['in_view'] == 1) & (df['connected'] == 0)]
        
        # Each entry represents 10 seconds
        contention_seconds = len(contention_entries) * 10
        contention_hours = contention_seconds / 3600
        
        return contention_hours
...
def collect_all_data() -> pd.DataFrame:
    """
    Collect contention time data for all configurations.
    ...
    for _, config in configs_df.iterrows():
        ...
        for policy in POLICIES:
            contention_hours = get_contention_time(config['zip_path'], policy)
...
def create_stacked_bar_chart(df: pd.DataFrame, image_size_kb: int) -> None:
    ...
    for x_pos, label in zip(x_positions, x_labels):
        ...
        for policy in POLICIES:
            ...
            if not policy_data.empty:
                value = policy_data['contention_hours'].values[0]
    ...
    ax.set_ylabel('Total Contention Time (hours)', fontsize=14, fontweight='bold')
    ax.set_title(f'Contention Time by Strategy & Policy\nImage Size: {image_size_mb:.3f} MB', 
    ...
    # Save
    output_file = OUTPUT_DIR / f"stacked_contention_time_{image_size_mb:.3f}mb.png"
...
def main():
    print("=" * 100)
    print("=" * 18 + " " * 64 + "=" * 18)
    print("=" * 18 + " " * 64 + "CONTENTION TIME STACKED BAR CHARTS")
    ...
    # Save raw data
    csv_output = OUTPUT_DIR / "stacked_contention_time.csv"
