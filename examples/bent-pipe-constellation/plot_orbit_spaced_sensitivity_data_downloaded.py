...
ORBIT-SPACED STRATEGY ONLY - SENSITIVITY STUDY

This shows total data downloaded to understand performance across constellation sizes.

X-axis: Constellation sizes (1, 10, 15, 17, 18, 19, 20, 21, 22, 25, 50, 100, 200)
...
def scan_orbit_spaced_sensitivity_configurations(search_dirs=['results/orbit space sensitivity 2', 'results/base results 2']):
    """Scan for orbit-spaced configurations in sensitivity study and base results directories"""
    configs = []
...
def create_orbit_spaced_sensitivity_charts():
    ...
    print("Sensitivity Study + Base Results | Stacked by Policy")
    print("="*110)
    print()
    
    print("Scanning for orbit-spaced configurations (sensitivity + base results)...")
    configs_df = scan_orbit_spaced_sensitivity_configurations()
...
def create_charts(results_df, output_dir):
    """Create stacked bar charts per image size (orbit-spaced only)"""
    ...
    for image_size in image_sizes:
        print(f"Creating orbit-spaced data downloaded chart for {image_size} MB...")
        ...
        # Title
        ax.set_title(f'Orbit-Spaced: Data Downloaded by Satellite Count\n{image_size} MB Images',
        ...
        # Save
        filename = f'orbit_spaced_sensitivity_data_downloaded_{image_size}mb.png'
    ...
    print()
    print("="*110)
    print("✅ ORBIT-SPACED SENSITIVITY DATA DOWNLOADED STACKED BAR CHARTS COMPLETE!")
    print("="*110)
    print()
    print("Chart Structure:")
    print("  X-AXIS: Constellation sizes (varies by data availability)")
...
if __name__ == '__main__':
    ...
    results_df = create_orbit_spaced_sensitivity_charts()
