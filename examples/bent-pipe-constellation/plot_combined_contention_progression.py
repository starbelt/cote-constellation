#!/usr/bin/env python3
"""
Create combined contention progression charts.
...
- Left: Missed Opportunity Rate (contention)
- Right: Connection Efficiency
...
def get_contention_metrics(zip_path, policy='fifo'):
    """Get contention metrics from visibility log"""
    try:
        with zipfile.ZipFile(zip_path, 'r') as zipf:
            with zipf.open(f'{policy}/visibility_log.csv') as f:
                df = pd.read_csv(f)
        
        # Total times satellite was in view
        total_in_view = len(df[df['in_view'] == 1])
        
        # Times connected when in view
        connected_when_in_view = len(df[(df['in_view'] == 1) & (df['connected'] == 1)])
        
        # Times missed when in view
        missed_when_in_view = len(df[(df['in_view'] == 1) & (df['connected'] == 0)])
        
        if total_in_view == 0:
            return None
        
        # Calculate percentages
        connection_efficiency = (connected_when_in_view / total_in_view) * 100
        missed_opportunity_rate = (missed_when_in_view / total_in_view) * 100
        
        return {
            'total_in_view': total_in_view,
            'connected_when_in_view': connected_when_in_view,
            'missed_when_in_view': missed_when_in_view,
...
def create_combined_charts():
    ...
    print("="*110)
    print("=" * 38 + " COMBINED CONTENTION PROGRESSION")
    ...
    for (image_size_kb, strategy, num_sats), group in grouped:
        ...
        for policy in policies:
            row = group.iloc[0]
            metrics = get_contention_metrics(row['zip_path'], policy)
    ...
    output_dir = SCRIPT_DIR / "constellation_analysis" / "contention_progression"
    output_dir.mkdir(parents=True, exist_ok=True)
    results_df.to_csv(output_dir / 'combined_contention_analysis.csv', index=False)
    print(f"✅ Saved: {output_dir / 'combined_contention_analysis.csv'}")
    print()
...
def create_charts(results_df, output_dir):
    ...
    for image_size in image_sizes:
        ...
        for strategy in strategies:
            for policy in policies:
                ...
                missed_vals = data['missed_opportunity_rate'].values
                efficiency_vals = data['connection_efficiency'].values
                ...
                if len(x_vals) > 0:
                    ...
                    # PANEL 1: Missed Opportunity Rate (Red theme - bad)
                    ax1.plot(x_vals, missed_vals, 
                    ...
                    # PANEL 2: Connection Efficiency (Green theme - good)
                    ax2.plot(x_vals, efficiency_vals, 
                            marker=policy_markers[policy], 
                            markersize=10,
                            linewidth=3,
                            linestyle=policy_styles[policy],
        ...
        # PANEL 1: Missed Opportunity Rate
        ax1.set_xlabel('Number of Satellites', fontsize=14, fontweight='bold')
        ax1.set_ylabel('In view not connected - Contention (%)', fontsize=14, fontweight='bold', color='darkred')
        ax1.set_title(f'In view not connected - Contention Rate (Lower is Better)\n{image_size} MB Images',
        ...
        # PANEL 2: Connection Efficiency
        ax2.set_xlabel('Number of Satellites', fontsize=14, fontweight='bold')
        ax2.set_ylabel('Connection Efficiency (%)', fontsize=14, fontweight='bold', color='darkgreen')
        ax2.set_title(f'Connection Efficiency (Higher is Better)\n{image_size} MB Images',
                     fontsize=15, fontweight='bold')
        ...
        # Save
        filename = f'combined_contention_{image_size}mb.png'
    ...
    print()
    print("="*110)
    print("✅ COMBINED CONTENTION CHARTS COMPLETE!")
    print("="*110)
    print()
    print("Chart Legend:")
    ...
    print("          2 panels per chart (missed opportunity + connection efficiency)")
