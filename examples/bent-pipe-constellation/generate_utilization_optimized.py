#!/usr/bin/env python3
"""
Generate optimized utilization clustered bar chart for 2.8 MB images
Based on professor feedback:
- Shorter and wider figure
- 5x larger fonts
- Satellite count labels below bars, above spacing labels
...
OUTPUT_DIR = BASE_DIR / 'design_comparison' / 'utilization_analysis'
SIMULATION_DURATION = 21600  # 6 hours in seconds
...
def extract_utilization(config, policy):
    """Extract utilization (1 - idle_fraction)"""
    ...
    try:
        with zipfile.ZipFile(zip_path, 'r') as z:
            ...
            with z.open(log_file) as f:
                ...
                # Calculate utilization (1 - idle_fraction)
                productive_mask = (df['connected'] == 1) & (df['buffer_mb'] > 0.001)
                productive_events = productive_mask.sum()
                idle_hours = (SIMULATION_DURATION - productive_events) / 3600.0
                idle_fraction = idle_hours / (SIMULATION_DURATION / 3600.0)
                utilization = 1.0 - idle_fraction
...
def create_optimized_clustered_bar_chart(df):
    ...
    for idx, policy in enumerate(policies):
        ...
        for i, sat_count in enumerate(sat_counts):
            ...
            for j, strategy in enumerate(strategies_ordered):
                ...
                if len(subset) > 0:
                    util_val = subset['utilization'].values[0] * 100
            ...
            bars = ax.bar(x + offset, utilizations, width,
                         label=f'{sat_count} Satellites',
                         color=colors[i],
                         edgecolor='black', linewidth=2, alpha=0.9)
            
            # Add utilization values on top of bars
            for bar, util_val in zip(bars, utilizations):
        ...
        # Formatting
        ax.set_ylabel('Utilization (%)', fontsize=28, fontweight='bold')
    ...
    # Main title removed or simplified - professor wants cleaner look
    plt.tight_layout()
    return fig

def main():
    print("="*70)
    print("OPTIMIZED UTILIZATION PLOT: 2.8 MB Images")
    print("="*70)
    ...
    for config in configs:
        for policy in policies:
            utilization = extract_utilization(config, policy)
    ...
    output_path = OUTPUT_DIR / 'utilization_clustered_optimized.png'
    ...
    print("   ✓ 5x larger fonts (40-55pt)")
    print("   ✓ Satellite count labels below bars")
    print("   ✓ Removed 'Spaced' redundancy from labels")
    print("   ✓ Utilization values displayed on top of bars")
