...
def create_bar_chart_for_strategy(strategy, buffer_results, config, output_dir, constellation_name):
    ...
    # Save the plot with constellation name in filename
    output_filename = f"buffer_bars_{constellation_name}_{strategy}_strategy.png"
    ...
    if any(totals):
        ...
        if ranking:
            print(f"     📊 {strategy_display} ranking: {ranking}")
...
def create_bar_charts(folder_path=None):
    ...
    # Save to constellation_analysis/buffer_bars/ directory
    output_dir = SCRIPT_DIR / "constellation_analysis" / "buffer_bars"
    output_dir.mkdir(parents=True, exist_ok=True)
    
    # Extract constellation info from folder name for filenames
    constellation_name = constellation_folder.name
    ...
    for strategy in SPACING_STRATEGIES:
        ...
        create_bar_chart_for_strategy(strategy, buffer_results, config, output_dir, constellation_name)
